...
// 把我的思念，化作一串珍珠送给你
func MakeWriteCmd(v variable.T) []byte {
	// 拾起一颗颗珍珠
	data := make([]byte, 16)
...
// 把我的思念，化作一串珍珠送给你
func MakeCmd(act ActMode, v variable.CmdT) []byte {
...
func FindValidPart(data []byte) (int, int) {
	...
	// 苦苦寻找你的痕迹
	for r < n && (data[l] != Board1 || ActMode(data[l+1]) != SubscribeReturn || data[r] != '\n') {
		l++
		r++
	}
